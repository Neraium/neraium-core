"""Turbo FD004 validation runner for fast iteration.

This is a TURBO/SUBSET runner for quick validation during development.
NOT the official benchmark. Results are directional, not final numbers.

Uses the same StructuralEngine and output schema as the canonical runner,
but processes fewer units/cycles for fast iteration.

Usage:
    # 5-unit smoke test (default)
    python -m runners.run_fd004_turbo

    # 10-unit validation
    python -m runners.run_fd004_turbo --max-units 10

    # 5-unit turbo run with cycle limit per unit
    python -m runners.run_fd004_turbo --max-units 5 --max-cycles 150

    # With explicit data paths
    python -m runners.run_fd004_turbo \\
        --test test_FD004.txt --rul RUL_FD004.txt \\
        --max-units 8 --output-dir outputs/my_turbo_runs

    # Skip charts for faster runs
    python -m runners.run_fd004_turbo --max-units 10 --no-charts

Output:
    Generates results in outputs/turbo_benchmarks/ (or custom --output-dir):
    - FD004_turbo_<timestamp>.csv (detailed results per frame)
    - FD004_turbo_scored_<timestamp>.csv (unit-level summary with RUL)
    - FD004_turbo_summary_<timestamp>.json (metrics)
    - FD004_turbo_*_<timestamp>.png (optional charts if not --no-charts)

Schema compatibility:
    Same columns and field names as canonical runner.
    Results can be compared row-by-row if same data subset used.
"""
...
# Default turbo limits for fast iteration
DEFAULT_MAX_UNITS = 5
DEFAULT_MAX_CYCLES = None  # No per-unit cycle limit by default

# Output directory (turbo location, separate from canonical)
OUTPUT_DIR_BASE = Path("outputs") / "turbo_benchmarks"
...
# Will be set based on command-line args
OUTPUT_DIR: Path | None = None
RESULTS_CSV: Path | None = None
SCORED_CSV: Path | None = None
SUMMARY_JSON: Path | None = None
LEAD_HIST_PNG: Path | None = None
TIMELINE_PNG: Path | None = None
HERO1_PNG: Path | None = None
HERO2_PNG: Path | None = None

# ============================================================
# ENGINE CONFIGURATION (FD004 locked policy - SAME AS CANONICAL)
# ============================================================
...
# ============================================================
# HELPERS (shared with canonical - could be factored out)
# ============================================================
...
def resolve_data_paths(test_arg: str | None = None, rul_arg: str | None = None) -> tuple[Path, Path]:
    ...
    raise FileNotFoundError(
        ...
        f"  • python -m runners.run_fd004_turbo --test <path> --rul <path>\n"
...
def run_engine(df: pd.DataFrame) -> list[dict[str, Any]]:
    ...
    Same implementation as canonical runner.
    Uses real StructuralEngine - no shortcuts or fake paths.
...
def generate_charts(results_df: pd.DataFrame, scored_df: pd.DataFrame) -> None:
    ...
    plt.title("FD004 TURBO Lead Time Distribution")
    plt.xlabel("Cycles Before Failure")
    plt.ylabel("Count")
    plt.legend()
    plt.tight_layout()
    plt.savefig(LEAD_HIST_PNG, dpi=150)
    plt.close()
    ...
    if len(timeline_df) > 0:
        ...
        plt.title("FD004 TURBO Alert to Failure Timeline")
        plt.xlabel("Cycle")
        plt.ylabel("Units")
        plt.legend()
        plt.tight_layout()
    ...
    # Hero units
    hero_df = timeline_df.sort_values("alert_lead").reset_index(drop=True) if len(timeline_df) > 0 else pd.DataFrame()
    hero_units = []
    if len(hero_df) >= 2:
        hero_units = [
    ...
    for i, unit in enumerate(hero_units[:2]):
        ...
        plt.title(f"FD004 TURBO Hero Unit {int(unit)}")
        plt.xlabel("Cycle")
        plt.ylabel("Structural Drift Score")
        plt.legend()
        plt.tight_layout()
        plt.savefig(hero_paths[i], dpi=150)
        plt.close()
...
def print_summary(summary: dict[str, Any], scored_df: pd.DataFrame) -> None:
    ...
    """
    print("\n" + "=" * 70)
    print("FD004 TURBO BENCHMARK RESULTS")
    print("(Directional validation run - not official benchmark)")
    print("=" * 70)
...
def main() -> None:
    """Run turbo FD004 validation."""
    parser = argparse.ArgumentParser(
        description=(
            "Turbo FD004 validation runner for fast iteration.\n"
            "Uses same StructuralEngine and schema as canonical, but reduced data.\n"
            "Results are directional - not official benchmark."
        ),
        ...
        epilog=(
            ...
            "  python -m runners.run_fd004_turbo --max-units 10\n"
            "\n"
            "  # 5-unit run with cycle limit\n"
            "  python -m runners.run_fd004_turbo --max-units 5 --max-cycles 150\n"
            "\n"
            "  # Custom output directory\n"
            "  python -m runners.run_fd004_turbo --max-units 8 --output-dir outputs/my_turbo_runs\n"
            "\n"
            "  # Skip charts for faster runs\n"
            "  python -m runners.run_fd004_turbo --max-units 10 --no-charts\n"
        ),
    ...
    parser.add_argument(
        ...
        help=f"Maximum number of units to process (default: {DEFAULT_MAX_UNITS})",
    )
    parser.add_argument(
        "--max-cycles",
        type=int,
        default=DEFAULT_MAX_CYCLES,
        help="Maximum cycles per unit to process (default: None = all cycles)",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default=None,
        help=f"Custom output directory (default: {OUTPUT_DIR_BASE})",
    )
    parser.add_argument(
        "--no-charts",
        action="store_true",
    ...
    global TEST_FILE, RUL_FILE, OUTPUT_DIR
    global RESULTS_CSV, SCORED_CSV, SUMMARY_JSON
    global LEAD_HIST_PNG, TIMELINE_PNG, HERO1_PNG, HERO2_PNG
    ...
    # Set output paths (turbo naming scheme)
    RESULTS_CSV = OUTPUT_DIR / f"FD004_turbo_{TIMESTAMP}.csv"
    SCORED_CSV = OUTPUT_DIR / f"FD004_turbo_scored_{TIMESTAMP}.csv"
    SUMMARY_JSON = OUTPUT_DIR / f"FD004_turbo_summary_{TIMESTAMP}.json"
    LEAD_HIST_PNG = OUTPUT_DIR / f"FD004_turbo_lead_time_{TIMESTAMP}.png"
    TIMELINE_PNG = OUTPUT_DIR / f"FD004_turbo_timeline_{TIMESTAMP}.png"
    HERO1_PNG = OUTPUT_DIR / f"FD004_turbo_hero_1_{TIMESTAMP}.png"
    HERO2_PNG = OUTPUT_DIR / f"FD004_turbo_hero_2_{TIMESTAMP}.png"

    print("=" * 70)
    print("TURBO FD004 VALIDATION RUNNER")
    print("(Directional validation - not official benchmark)")
    print("=" * 70)
    print(f"\nResolved paths:")
    ...
    print(f"  Max Cycles per Unit: {args.max_cycles or 'None (all)'}")
    ...
    units_to_process = unique_units[:args.max_units]
    df = df[df["unit"].isin(units_to_process)].copy()
