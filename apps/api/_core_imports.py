...
# Prefer app-local modules when present (vendored runtime modules).
try:
    from .csv_mapping import (  # type: ignore[attr-defined]
    ...
    try:
        from csv_mapping import (  # type: ignore[no-redef]
            infer_semantic_mapping,
            parse_csv_sample_for_mapping,
            resolve_mapping,
            row_to_frame_kwargs,
...
try:
    from .logging_utils import log_structured, summarize_exception_for_logs  # type: ignore[attr-defined]
except ImportError:
    try:
        from logging_utils import log_structured, summarize_exception_for_logs  # type: ignore[no-redef]
    except ImportError:
        import logging
...
try:
    from .store import ResultStore  # type: ignore[attr-defined]
except ImportError:
    try:
        from store import ResultStore  # type: ignore[no-redef]
    except ImportError:
        class ResultStore:
...
try:
    from .service import StructuralMonitoringService  # type: ignore[attr-defined]
except ImportError:
    try:
        from service import StructuralMonitoringService  # type: ignore[no-redef]
    except ImportError:
