...
"""Runtime imports for the API app that work across local/dev and slim images.

Prefer canonical ``neraium_core.*`` modules when they are available (standard local
development). Fall back to app-local/shim modules when running images that only
mount ``apps/api``.
"""
