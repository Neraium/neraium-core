...
def build_integrations_router(*, app: Any, require_api_key: Any, resolve_customer_id: Any, resolve_run_id_with_default: Any, service_instance: Any, stop_pull_integration: Any, start_pull_worker: Any, public_pull_state: Any, pull_integrations: dict[str, Any], pull_integrations_lock: Any, persist_pull_state: Any, validate_endpoint_url: Any, parse_finite_float: Any, parse_int: Any, utc_now_iso: Any, log_structured: Any, models: Any) -> APIRouter:
    ...
    def start_pull_integration(payload: models.PullIntegrationStartRequest, _: None = Depends(require_api_key), customer_id: str | None = Query(default=None)) -> dict[str, Any]:
        ...
        endpoint_url = validate_endpoint_url(payload.endpoint_url or str(resolved_cfg.get("endpoint_url") or ""))
        ...
        polling_interval_seconds = parse_finite_float(payload.polling_interval_seconds if payload.polling_interval_seconds is not None else resolved_cfg.get("polling_interval_seconds") or 30.0, field_name="polling_interval_seconds")
        retry_max_attempts = parse_int(payload.retry_max_attempts if payload.retry_max_attempts is not None else resolved_cfg.get("retry_max_attempts") or 3, field_name="retry_max_attempts")
        retry_backoff_seconds = parse_finite_float(payload.retry_backoff_seconds if payload.retry_backoff_seconds is not None else resolved_cfg.get("retry_backoff_seconds") or 1.0, field_name="retry_backoff_seconds")
        request_timeout_seconds = parse_finite_float(payload.request_timeout_seconds if payload.request_timeout_seconds is not None else resolved_cfg.get("request_timeout_seconds") or 10.0, field_name="request_timeout_seconds")
        ...
        stop_pull_integration(resolved_customer, reason="Restarting integration.")
        stop_event = threading.Event()
        started_at = utc_now_iso()
        with pull_integrations_lock:
            pull_integrations[resolved_customer] = {"customer_id": resolved_customer, "endpoint_url": endpoint_url, "run_id": resolved_run, "auth_type": auth_type, "username": username, "password": password, "token": token, "running": True, "status": "running", "polling_interval_seconds": polling_interval_seconds, "retry_max_attempts": retry_max_attempts, "retry_backoff_seconds": retry_backoff_seconds, "request_timeout_seconds": request_timeout_seconds, "started_at": started_at, "updated_at": started_at, "last_poll_at": None, "last_success_at": None, "last_error": None, "last_http_status": None, "total_polls": 0, "total_failures": 0, "consecutive_failures": 0, "total_ingested": 0, "message": "Pull integration started.", "_stop_event": stop_event, "_thread": None}
            state = dict(pull_integrations[resolved_customer])
        persist_pull_state(resolved_customer, state)
        start_pull_worker(resolved_customer)
        ...
        return public_pull_state(state, customer_id=resolved_customer)
    ...
    def stop_pull(_: None = Depends(require_api_key), customer_id: str | None = Query(default=None)) -> dict[str, Any]:
        resolved_customer = resolve_customer_id(customer_id)
        state = stop_pull_integration(resolved_customer, reason="Pull integration stopped by operator.")
        return state
