...
from .dependencies import IntegrationsRouterDependencies

logger = logging.getLogger(__name__)


def build_integrations_router(*, deps: IntegrationsRouterDependencies) -> APIRouter:
    router = APIRouter(tags=["integrations"])

    @router.post("/integrations/pull/start", response_model=PullIntegrationStatusEnvelope)
    def start_pull_integration(payload: PullIntegrationStartRequest, _: None = Depends(deps.require_api_key), customer_id: str | None = Query(default=None)) -> dict[str, Any]:
        resolved_customer = deps.resolve_customer_id(customer_id)
        cfg_override = getattr(deps.app.state, "integration_config_override", None)
        if isinstance(cfg_override, dict):
            cfg_doc = cfg_override
        else:
            path_override = getattr(deps.app.state, "integration_config_path_override", None)
        ...
        endpoint_url = deps.pull_manager.validate_endpoint_url(payload.endpoint_url or str(resolved_cfg.get("endpoint_url") or ""))
        ...
        polling_interval_seconds = deps.pull_manager.parse_finite_float(payload.polling_interval_seconds if payload.polling_interval_seconds is not None else resolved_cfg.get("polling_interval_seconds") or 30.0, field_name="polling_interval_seconds")
        retry_max_attempts = deps.pull_manager.parse_int(payload.retry_max_attempts if payload.retry_max_attempts is not None else resolved_cfg.get("retry_max_attempts") or 3, field_name="retry_max_attempts")
        retry_backoff_seconds = deps.pull_manager.parse_finite_float(payload.retry_backoff_seconds if payload.retry_backoff_seconds is not None else resolved_cfg.get("retry_backoff_seconds") or 1.0, field_name="retry_backoff_seconds")
        request_timeout_seconds = deps.pull_manager.parse_finite_float(payload.request_timeout_seconds if payload.request_timeout_seconds is not None else resolved_cfg.get("request_timeout_seconds") or 10.0, field_name="request_timeout_seconds")
        ...
        resolved_run = deps.resolve_run_id_with_default(deps.service_instance, payload.run_id, customer_id=resolved_customer)
        state = deps.pull_manager.start_pull_integration(
    ...
    def stop_pull(_: None = Depends(deps.require_api_key), customer_id: str | None = Query(default=None)) -> dict[str, Any]:
        resolved_customer = deps.resolve_customer_id(customer_id)
        return deps.pull_manager.stop_pull_integration(resolved_customer, reason="Pull integration stopped by operator.")

    @router.get("/integrations/pull/status", response_model=PullIntegrationStatusEnvelope)
    def pull_status(customer_id: str | None = Query(default=None)) -> dict[str, Any]:
        resolved_customer = deps.resolve_customer_id(customer_id)
        with deps.pull_manager.store.pull_integrations_lock:
            state = deps.pull_manager.store.pull_integrations.get(resolved_customer)
            return deps.pull_manager.public_pull_state(state, customer_id=resolved_customer)
