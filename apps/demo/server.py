...
def build_sensor_values() -> dict[str, float]:
    base_pressure = 60
    base_flow = 125
    base_tank = 75
    base_quality = 97
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/status":
            with lock:
                out = dict(events[-1]) if events else {
                    "state": "UNKNOWN",
                    "timestamp": "-",
                    "site_id": "—",
                    "asset_id": "—",
        ...
        if path in {"/api/scenario/normal", "/api/scenario/degrading", "/api/scenario/incident"}:
            with lock:
                scenario = path.rsplit("/", maxsplit=1)[-1]
