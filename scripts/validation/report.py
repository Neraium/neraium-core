...
"""
    ...
    report += "\n## Notes\n\n"
    report += "- **Lead Time:** Computed as cycles from first drift threshold breach to end of run.\n"
    report += "- **Structural Drift Score:** Primary indicator of system degradation.\n"
    report += "- **Relational Instability Score:** Secondary indicator based on sensor relationships.\n"
    report += "- **Early Signal Threshold:** Default 0.5 (configurable).\n"

    report += "\n## Plots\n\n"
    report += "For each dataset, three representative plots are generated:\n\n"
    report += "- **Best Case:** Unit with shortest lead time (fastest to failure)\n"
    report += "- **Median Case:** Unit at 50th percentile of lead time distribution\n"
    report += "- **Worst Case:** Unit with longest lead time (slowest degradation)\n\n"
