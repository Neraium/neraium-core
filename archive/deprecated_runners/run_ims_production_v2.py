...
df.to_csv("IMS_production_results_v2.csv", index=False)

plt.figure(figsize=(12, 6))
plt.plot(df["t"], df["drift_smooth"])
plt.title("IMS Drift (Production Smoothed v2)")
plt.xlabel("Time")
plt.ylabel("Drift Score")
plt.tight_layout()
plt.savefig("ims_prod_v2_drift.png", dpi=200)
plt.show()
...
plt.plot(df["t"], df["drift_smooth"])
plt.title("IMS Drift + Stable States v2")
plt.xlabel("Time")
plt.ylabel("Drift Score")
plt.tight_layout()
plt.savefig("ims_prod_v2_overlay.png", dpi=200)
plt.show()

plt.figure(figsize=(12, 4))
plt.plot(df["t"], df["state_num"])
plt.yticks([0, 1, 2], ["STABLE", "WATCH", "ALERT"])
plt.title("IMS Clean State Transitions v2")
plt.xlabel("Time")
plt.ylabel("State")
plt.tight_layout()
plt.savefig("ims_prod_v2_states.png", dpi=200)
plt.show()
...
print("\nSaved:", flush=True)
print(" - IMS_production_results_v2.csv", flush=True)
print(" - ims_prod_v2_drift.png", flush=True)
print(" - ims_prod_v2_overlay.png", flush=True)
print(" - ims_prod_v2_states.png", flush=True)
