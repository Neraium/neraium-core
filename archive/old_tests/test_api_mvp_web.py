...
def test_mvp_routes_available(tmp_path) -> None:
    ...
    assert any(
        token in three_init.headers.get("content-type", "")
        for token in ("text/javascript", "application/javascript")
    )
...
def test_web_app_fetch_recent_results_uses_results_recent_endpoint(tmp_path) -> None:
    ...
    assert js.status_code == 200
    source = js.text
    assert 'apiUrl("/runs", recentParams)' in source
    assert 'apiUrl("/results/recent", recentParams)' not in source
