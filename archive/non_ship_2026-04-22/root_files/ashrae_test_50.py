...
for (b, m), g in df.groupby(["building_id", "meter"]):
    ...
    print(f"Testing unit {tested}/{MAX_UNITS}: building={b}, meter={m}")
...
pd.DataFrame(
    valid,
    columns=["building_id", "meter", "warning_index", "n_cycles", "warning_ratio"]
).to_csv("ashrae_warning_results_50.csv", index=False)

print("\nSaved: ashrae_warning_results_50.csv")
