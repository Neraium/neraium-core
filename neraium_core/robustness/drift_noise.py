...
def classify_drift_noise(drift_history: list[float]) -> Dict[str, object]:
    ...
    if arr.size < 3:
        return {
            ...
            "acceleration_persistence_score": 0.0,
    ...
    accel = np.diff(diffs) if diffs.size >= 2 else np.asarray([0.0], dtype=float)
    accel_persistence = float(np.mean(np.maximum(0.0, accel)))

    oscillatory_score = float(np.mean(np.sign(diffs[1:]) != np.sign(diffs[:-1]))) if diffs.size >= 2 else 0.0

    if oscillatory_score > 0.6 and transient > 0.08:
        interpreted = "oscillatory_instability"
    elif persistence > 1.1 and np.mean(diffs[-min(5, diffs.size):]) > 0.02:
    ...
    return {
        ...
        "acceleration_persistence_score": round(accel_persistence, 4),
