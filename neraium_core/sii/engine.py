...
from neraium_core.sii.decision import map_decision_state, map_to_interpreted_state
...
class SystemicInfrastructureIntelligenceEngine:
    ...
    def _ensure_sensor_order(self, frame: TelemetryFrame) -> None:
        if not self.state.sensor_order:
            self.state.sensor_order = sorted(frame.sensor_values.keys())
    ...
    def process_frame(self, frame: TelemetryFrame) -> SIIResult:
        ...
        self.state.timestamps.append(float(frame.timestamp))
        ...
        regime_distance = float(reg_assign.regime_distance) if reg_assign.regime_distance is not None else float(
            self.config.regime_distance_threshold
        )
        ...
        trend = self._trend()
        decision_state = map_decision_state(
            composite_score=float(composite),
            interpreted_state=interpreted,
            trend=trend,
            stability=self._classification_stability(),
        )
        ...
        driver_names = [k for k, _ in drivers]
        context = None
        ...
        out: SIIResult = {
            ...
            "explanation": (
                f"{interpreted}: multivariate geometry and graph topology indicate "
                f"system-level structural behavior {'departure' if decision_state != 'STABLE' else 'consistency'}."
            ),
            "read_only": True,
            "experimental_analytics": {
