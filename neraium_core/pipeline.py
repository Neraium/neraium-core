...
class TelemetryPipeline:
    ...
    def _update_baseline(self, vector: list[float]) -> None:
        ...
        if len(self.history) < self.training_samples:
            return
        data = np.array(self.history[-self.training_samples :], dtype=float)
    ...
    def process(self, payload: Any) -> dict[str, Any]:
        cpu = float(payload.signals["cpu_usage"])
        mem = float(payload.signals["memory_usage"])
        f = self.features.compute(cpu, mem)
        vector = [f["cpu"], f["memory"], f["cpu_delta"], f["mem_delta"], f["cpu_std"], f["mem_std"]]
        self._update_baseline(vector)
        score = self._mahalanobis(vector)
        return {
            "timestamp": datetime.now(timezone.utc).isoformat(),
            "signals": {"cpu_usage": cpu, "memory_usage": mem},
            "score": score,
            "status": "anomaly" if score > 4 else "normal",
        }
>>>>>>> origin/main
