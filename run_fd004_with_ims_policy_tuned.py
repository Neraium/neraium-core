...
for unit, unit_df in df.groupby("unit", sort=True):
    ...
    alert_threshold = float(unit_df["drift_smooth"].quantile(0.85))
...
policy_df.to_csv("FD004_ims_policy_tuned_results.csv", index=False)
...
summary.to_csv("FD004_ims_policy_tuned_scored.csv", index=False)

lead = summary.loc[summary["has_alert"], "alert_lead"]

print("\n" + "=" * 60)
print("FD004 WITH TUNED IMS POLICY")
print("=" * 60)
...
print("\nSaved:")
print(" - FD004_ims_policy_tuned_results.csv")
print(" - FD004_ims_policy_tuned_scored.csv")
