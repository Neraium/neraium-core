...
def test_warmup_excludes_early_cycles() -> None:
    ...
    cfg = TransitionDetectionConfig(warmup_cycles=40.0, threshold=0.5, sustained_points=2, smoothing_window=3)
...
def test_constant_signal_no_detection() -> None:
    ...
    r = detect_transition(df, TransitionDetectionConfig(warmup_cycles=1.0))
...
def test_missing_values_interpolated() -> None:
    ...
    p[10:15] = np.nan
    df = _cycles_df(len(p), p)
    r = detect_transition(df, TransitionDetectionConfig(warmup_cycles=1.0, threshold=0.5, sustained_points=2))
    assert r["detected"] is True
...
def test_fallback_max_slope_when_no_crossing() -> None:
    ...
    df = _cycles_df(len(p), p)
    cfg = TransitionDetectionConfig(warmup_cycles=1.0, threshold=0.99, sustained_points=5, fallback_mode="max_slope")
    r = detect_transition(df, cfg)
    assert r["detected"] is True
...
def test_fallback_none() -> None:
    p = np.linspace(0.1, 0.15, 50)
    df = _cycles_df(len(p), p)
    cfg = TransitionDetectionConfig(warmup_cycles=1.0, threshold=0.99, sustained_points=5, fallback_mode="none")
    r = detect_transition(df, cfg)
    assert r["detected"] is False
...
def test_grouped_multi_entity() -> None:
    ...
    cfg = TransitionDetectionConfig(warmup_cycles=5.0, entity_group_column="unit")
...
def test_summarize_transition_detection_api() -> None:
    ...
    cfg = TransitionDetectionConfig(index_column="cycle", warmup_cycles=1.0, entity_group_column="asset")
...
def test_detect_transition_from_signal() -> None:
    ...
    r = detect_transition_from_signal(sig, config=TransitionDetectionConfig(warmup_cycles=0.0, threshold=0.5))
    assert r["detected"] is True
...
@pytest.mark.parametrize("warmup_cycles", [0.0, 1.0])
def test_insufficient_history(warmup_cycles: float) -> None:
    df = pd.DataFrame({"cycle": [1.0, 2.0], "transition_pressure": [0.1, 0.2]})
    cfg = TransitionDetectionConfig(min_history=10, warmup_cycles=warmup_cycles)
    r = detect_transition(df, cfg)
    assert r["detected"] is False
    assert r["detection_reason"] == "insufficient_post_warmup"
