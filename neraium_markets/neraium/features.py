...
def _resolve_asset_column(df: pd.DataFrame, asset: str) -> str | None:
    """Return the price column name for an asset, handling uppercase alignment output."""
    for candidate in (asset, asset.upper(), asset.lower()):
...
def compute_returns(df: pd.DataFrame, assets: list[str], windows: list[int] = [1, 5]) -> pd.DataFrame:
    ...
    out = df.copy()
    for asset in assets:
        price_col = _resolve_asset_column(out, asset)
        if price_col is None:
            continue
        for window in windows:
            out[f"{asset}_ret_{window}d"] = out[price_col].pct_change(periods=window)
    return out
...
def compute_breadth_features(df: pd.DataFrame, sector_assets: list[str]) -> pd.DataFrame:
    ...
    for asset in sector_assets:
        ...
        if price_col is None:
            continue
        ma20 = out[price_col].rolling(window=20).mean()
        above_cols.append((out[price_col] > ma20).astype(float))
...
def compute_cross_asset_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    us10y_col = _resolve_asset_column(out, "us10y")
    us2y_col = _resolve_asset_column(out, "us2y")
    if us10y_col and us2y_col:
        out["rates_2s10s"] = out[us10y_col] - out[us2y_col]
    else:
